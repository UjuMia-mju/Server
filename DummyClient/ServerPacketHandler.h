...
public:
	static void Init()
	{
		...
		{
			return HandlePacket<Protocol::S_GACHA>(Handle_S_GACHA, session, buffer, len);
		};
		GPacketHandler[PKT_S_GACHA_POOL_LIST] = [](PacketSessionRef& session, BYTE* buffer, int32 len)
		{
			return HandlePacket<Protocol::S_GACHA_POOL_LIST>(Handle_S_GACHA_POOL_LIST, session, buffer, len);
		};
		GPacketHandler[PKT_S_SKIN_LIST] = [](PacketSessionRef& session, BYTE* buffer, int32 len)
		{
			return HandlePacket<Protocol::S_SKIN_LIST>(Handle_S_SKIN_LIST, session, buffer, len);
		};
		GPacketHandler[PKT_S_MY_SKINS] = [](PacketSessionRef& session, BYTE* buffer, int32 len)
		{
			return HandlePacket<Protocol::S_MY_SKINS>(Handle_S_MY_SKINS, session, buffer, len);
		};
	...
	{
		return MakeSendBuffer(pkt, PKT_C_GACHA);
	}
	static SendBufferRef MakeSendBuffer(Protocol::C_GACHA_POOL_LIST& pkt)
	{
		return MakeSendBuffer(pkt, PKT_C_GACHA_POOL_LIST);
	}
	static SendBufferRef MakeSendBuffer(Protocol::C_MY_SKINS& pkt)
	{
		return MakeSendBuffer(pkt, PKT_C_MY_SKINS);
	}
